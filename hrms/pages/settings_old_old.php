...
require_role(ROLE_ADMIN);dadasdaddadsa
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($section === 'time') {
        ...
        // 4. Reset to real time — zero the delta, keep everything else
        if (isset($_POST['reset_clock']) && $_POST['reset_clock'] === '1') {
            $new_delta = 0;
            // Preserve existing shift settings (don't overwrite with empty)
            $pdo->prepare("INSERT INTO settings(setting_key,setting_value) VALUES('clock_delta_seconds','0') ON DUPLICATE KEY UPDATE setting_value='0'")->execute();
            date_default_timezone_set($tz);
            audit_log($pdo, 'UPDATE', 'settings', 0, array('action'=>'reset_clock','timezone'=>$tz));
    ...
    <div class="card">
      ...
      <div class="card-body">
        ...
        <form method="POST" action="" id="timeForm">
          ...
          <input type="hidden" name="reset_clock"  id="resetClock" value="0">
          ...
          <div class="card card-outline card-warning mb-3">
            <div class="card-header bg-light d-flex justify-content-between align-items-center">
              ...
              <div>
                ...
                <!-- Separate form — only sends reset signal, nothing else -->
                <form method="POST" action="settings.php?tab=time" style="display:inline-block;">
                  <?php csrf_field(); ?>
                  <input type="hidden" name="section"          value="time">
                  <input type="hidden" name="reset_clock"      value="1">
                  <input type="hidden" name="system_timezone"  value="<?= htmlspecialchars($_saved_tz) ?>">
                  <input type="hidden" name="adj_seconds"      value="0">
                  <input type="hidden" name="adj_minutes"      value="0">
                  <input type="hidden" name="adj_hours"        value="0">
                  <input type="hidden" name="adj_days"         value="0">
                  <input type="hidden" name="adj_months"       value="0">
                  <input type="hidden" name="adj_years"        value="0">
                  <input type="hidden" name="manual_datetime"  value="">
