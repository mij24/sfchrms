<?php
// ============================================
// FILE: pages/onboarding.php — Employee Lifecycle
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'assign') {
        ...
        $view_emp = $emp_id;
        prg_redirect("onboarding.php?employee_id=$emp_id", "Assigned $inserted task(s) to checklist.");
    }
    ...
    if ($action === 'update_task') {
        ...
        prg_redirect("onboarding.php?employee_id=" . clean_int($_POST['emp_id']), "Task updated.");
    }
}

$employees = db_fetchall($pdo,
    "SELECT e.id, e.full_name, e.employee_id, e.date_hired,
            d.name AS dept
     FROM employees e
...
<div class="content-wrapper">
  <div class="content-header"><div class="container-fluid">
    <div class="row mb-2">
      <div class="col-sm-6"><h1 class="m-0">Onboarding Checklist</h1></div>
      <div class="col-sm-6">
          ...
          <div class="card-body">
            <form method="POST" action="">
              ...
              <div class="form-group"><label>Employee</label>
                <select name="employee_id" class="form-control" required>
                  ...
                  <?php foreach ($employees as $e): ?>
                    <option value="<?= $e['id'] ?>" <?= $view_emp==$e['id']?'selected':'' ?>>
                      <?= htmlspecialchars($e['full_name']) ?>
                      <small>(<?= date('M d, Y', strtotime($e['date_hired'])) ?>)</small>
                    </option>
                  <?php endforeach; ?>
                </select>
              </div>
              ...
              <button type="submit" class="btn btn-primary btn-block">
                <i class="fas fa-tasks mr-1"></i> Assign Checklist
              </button>
            </form>
          </div>
        </div>
      ...
      <div class="col-md-8">
        ...
        <div class="card">
          <div class="card-header d-flex justify-content-between align-items-center">
            ...
            <span class="badge badge-<?= $pct_done>=100?'success':($pct_done>=50?'warning':'secondary') ?>">
              <?= $done_count ?>/<?= count($checklist) ?> done
            </span>
          </div>
          <div class="card-body">
            <div class="d-flex justify-content-between mb-1">
              <small class="text-muted">Progress</small>
              <small><strong><?= $pct_done ?>%</strong></small>
            </div>
            <div class="progress mb-4" style="height:8px;">
              <div class="progress-bar bg-<?= $pct_done>=100?'success':($pct_done>=50?'warning':'info') ?>"
                   style="width:<?= $pct_done ?>%"></div>
            </div>
            ...
            <?php foreach ($checklist as $task):
              ...
              $border_col = $task['status']==='Done' ? '#28a745' : ($task['status']==='In progress' ? '#ffc107' : '#dee2e6');
            ?>
            <div class="card mb-2" style="border-left:4px solid <?= $border_col ?>;">
              <div class="card-body py-2 px-3">
                <div class="d-flex justify-content-between align-items-start">
                  <div style="flex:1;">
                    <strong style="font-size:13px;"><?= htmlspecialchars($task['task_name']) ?></strong>
                    <span class="badge badge-<?= $rc ?> ml-1" style="font-size:10px;"><?= $task['responsible'] ?></span>
                    <?php if ($task['task_description']): ?>
                      <div><small class="text-muted"><?= htmlspecialchars($task['task_description']) ?></small></div>
                    <?php endif; ?>
                    <?php if ($task['completed_at']): ?>
                      <small class="text-success"><i class="fas fa-check mr-1"></i>Completed <?= date('M d, Y', strtotime($task['completed_at'])) ?></small>
                    <?php endif; ?>
                  </div>
                  <span class="badge badge-<?= $sc ?> ml-2"><?= $task['status'] ?></span>
                </div>
                <form method="POST" action="" class="mt-2 d-flex" style="gap:6px;">
                  <?php csrf_field(); ?>
                  <input type="hidden" name="action" value="update_task">
                  <input type="hidden" name="eo_id" value="<?= $task['id'] ?>">
                  <input type="hidden" name="emp_id" value="<?= $view_emp ?>">
                  ...
                  <input type="text" name="notes" class="form-control form-control-sm"
                         placeholder="Notes..." value="<?= htmlspecialchars($task['notes'] ?? '') ?>">
        ...
        <?php else: ?>
          <div class="card"><div class="card-body text-center text-muted py-5">
            ...
            Select an employee to view or assign their onboarding checklist.
          </div></div>
        <?php endif; ?>
      </div>
    </div>
