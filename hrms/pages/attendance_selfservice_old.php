...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_clock'])) {
    ...
    if ($action === 'in') {
        ...
        $msg = $status === 'Late'
            ? "Clocked in at ".date('h:i A')." — Late by $tardy minute(s)."
            : "Clocked in at ".date('h:i A').". Have a great day!";
        ...
        $msg = "Clocked out at ".date('h:i A').".";
        if ($ot_hrs > 0) $msg .= " Overtime: {$ot_hrs}h.";
        echo json_encode(array('ok'=>true,'msg'=>$msg,'time'=>date('h:i A')));
        exit;
    }
        ...
        <div class="card mb-3">
          <div class="card-body d-flex align-items-center" style="gap:14px;">
            ...
            <div class="ml-auto text-right">
              <?php if ($att_today && $att_today['time_in']): ?>
                <small class="text-success"><i class="fas fa-sign-in-alt mr-1"></i><?= date('h:i A', strtotime($att_today['time_in'])) ?></small>
              <?php endif; ?>
              <?php if ($att_today && $att_today['time_out']): ?>
                <br><small class="text-danger"><i class="fas fa-sign-out-alt mr-1"></i><?= date('h:i A', strtotime($att_today['time_out'])) ?></small>
              <?php endif; ?>
            </div>
          </div>
        </div>
        ...
        <div class="card clock-card">
          <div class="card-body">
            ...
            <div class="d-flex justify-content-between mb-3">
              <div>
                <div style="font-size:24px;font-weight:700;" id="liveTime"><?= date('h:i:s A') ?></div>
                <div class="timestamp"><?= date('l, F d, Y') ?></div>
              </div>
              <div class="text-right">
                <?php
                $can_in  = !$att_today || !$att_today['time_in'];
                $can_out = $att_today && $att_today['time_in'] && !$att_today['time_out'];
                ?>
                <span class="badge badge-<?= $can_in ? 'warning' : 'success' ?>" style="font-size:12px;">
                  <?= $can_in ? 'Not yet clocked in' : 'Clocked in' ?>
                </span>
              </div>
            </div>
            ...
            <div class="row" style="gap:0;">
              <div class="col-6 pr-1">
                <button id="btnClockIn" class="btn btn-success btn-clock btn-block"
                        <?= !$can_in ? 'disabled' : '' ?>
                        onclick="doClockAction('in')">
                  <i class="fas fa-sign-in-alt d-block mb-1" style="font-size:20px;"></i>
                  Clock In
                </button>
              </div>
              <div class="col-6 pl-1">
                <button id="btnClockOut" class="btn btn-danger btn-clock btn-block"
                        <?= !$can_out ? 'disabled' : '' ?>
                        onclick="doClockAction('out')">
                  <i class="fas fa-sign-out-alt d-block mb-1" style="font-size:20px;"></i>
                  Clock Out
                </button>
              </div>
            </div>
...
function doClockAction(action) {
    ...
    fetch('attendance_selfservice.php', { method:'POST', body:formData })
        ...
        .then(function(data) {
            ...
            if (data.ok) {
                // Reload after 2 seconds to refresh button states
